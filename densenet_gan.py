...
"""  
def DenseNet(input_tensor = None):
    
    model_in = input_tensor
    
    x = layers.ZeroPadding2D(padding = ((3, 3), (3, 3)))(model_in)
    d1_in = layers.Conv2D(64, 7, strides = 2, use_bias = False)(x)
    x = layers.BatchNormalization(axis = 3, epsilon = 1.001e-5)(d1_in)
    x = layers.Activation('relu')(x)
    x = layers.ZeroPadding2D(padding = ((1, 1), (1, 1)))(x)
    x_last = layers.MaxPooling2D(3, strides = 2)(x)
    
    d1 = dense_block(x_last, 6)
    d1 = transition_block(d1, 0.5)
    
    d2 = dense_block(d1, 12)
    d2 = transition_block(d2, 0.5)
    
    d3 = dense_block(d2, 24)
    d3 = transition_block(d3, 0.5)
    
    d4 = dense_block(d3, 16)
    
    
    x = layers.BatchNormalization(axis = 3, epsilon = 1.001e-5)(d4)
    x = layers.Activation('relu')(x)
    
    #notop
    x = layers.GlobalMaxPooling2D()(x)
    
    model = tf.keras.Model(inputs = [model_in], outputs = [x])
    model.load_weights('./densenet121_weights.h5')
    d1_block = tf.keras.Model(inputs = [d1_in], outputs = [d1_out])
...
def generator_model():
    ...
    u6 = layers.concatenate([padded, d3])
